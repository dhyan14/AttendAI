...
async def download_student_template(
    ...
    headers = ["name", "roll_no", "enrollment_no", "email", "division", "batch", "semester", "dept_id"]
    thin = Border(
    ...
    for r_idx, (col, desc) in enumerate([
        ...
        ("dept_id",        "Required. UUID of the department (copy from Super Admin panel)."),
        ("", ""),
...
async def bulk_import_students(
    ...
    try:
        df = pd.read_csv(buf, dtype=str) if ext == "csv" else pd.read_excel(buf, sheet_name="Students", dtype=str)
    ...
    if missing:
        raise HTTPException(status_code=422, detail=f"Missing required columns: {', '.join(sorted(missing))}. Use the downloaded template.")

    df.columns = df.columns.str.strip().str.lower()
    df = df.where(pd.notnull(df), None).head(500)

    results_ok: list[dict] = []
    results_err: list[dict] = []

    for row_num, row in df.iterrows():
        row_label = f"Row {int(row_num) + 2}"
        ...
        semester: Optional[int] = None
        if row.get("semester"):
            try: semester = int(float(str(row["semester"])))
            except ValueError: pass

        if not name:    results_err.append({"row": row_label, "email": email, "error": "'name' is required"}); continue
        if not roll_no: results_err.append({"row": row_label, "email": email, "error": "'roll_no' is required"}); continue
        if not email:   results_err.append({"row": row_label, "roll_no": roll_no, "error": "'email' is required"}); continue
        if not dept_id or dept_id.startswith("<"): results_err.append({"row": row_label, "email": email, "error": "'dept_id' is missing or still placeholder"}); continue

        try:
            dept_uuid = uuid.UUID(dept_id)
        except ValueError:
            results_err.append({"row": row_label, "email": email, "error": f"Invalid UUID: {dept_id}"}); continue

        dept = await db.get(Department, dept_uuid)
        if not dept: results_err.append({"row": row_label, "email": email, "error": f"Department not found: {dept_id}"}); continue

        ex_email = await db.execute(select(User).where(User.email == email))
        if ex_email.scalar_one_or_none(): results_err.append({"row": row_label, "email": email, "error": f"Email already exists"}); continue

        ex_roll = await db.execute(select(Student).where(Student.dept_id == dept_uuid, Student.roll_no == roll_no))
        if ex_roll.scalar_one_or_none(): results_err.append({"row": row_label, "email": email, "error": f"Roll No '{roll_no}' already exists in dept"}); continue

        try:
            user = User(email=email, password_hash=_hash("Student@123"), role=UserRole.student, org_id=dept.org_id, is_active=True)
            db.add(user)
            await db.flush()
            db.add(Student(user_id=user.id, name=name, roll_no=roll_no, enrollment_no=enroll, division=division, batch=batch, semester=semester, dept_id=dept_uuid))
            await db.flush()
    ...
    return {"total_rows": len(df), "success_count": len(results_ok), "error_count": len(results_err), "created": results_ok, "errors": results_err}
